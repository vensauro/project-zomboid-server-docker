...
if [ "${ARCH}" = "aarch64" ]; then
  ...
  export BOX64_DYNAREC_STRONGMEM="${BOX64_DYNAREC_STRONGMEM:-2}"
  export BOX64_DYNAREC_SAFEFLAGS="${BOX64_DYNAREC_SAFEFLAGS:-2}"
  export BOX64_DYNAREC_CALLRET="${BOX64_DYNAREC_CALLRET:-0}"
  export BOX64_DYNAREC_NATIVEFLAGS="${BOX64_DYNAREC_NATIVEFLAGS:-0}"
  # Keep HotSpot on its simplest compiler tier. Box64's DynaRec can then run
  # the generated code without the more aggressive tiered-compiler paths.
  ARGS="-XX:TieredStopAtLevel=1 -XX:-UseCompressedOops ${ARGS}"
  export LD_LIBRARY_PATH="${STEAMAPPDIR}/linux64:${STEAMAPPDIR}/natives:${LD_LIBRARY_PATH}"
  RUN_EXPORTS="${RUN_EXPORTS} BOX64_JVM=\"${BOX64_JVM}\" BOX64_DYNAREC=\"${BOX64_DYNAREC}\" BOX64_DYNAREC_BIGBLOCK=\"${BOX64_DYNAREC_BIGBLOCK}\" BOX64_DYNAREC_STRONGMEM=\"${BOX64_DYNAREC_STRONGMEM}\" BOX64_DYNAREC_SAFEFLAGS=\"${BOX64_DYNAREC_SAFEFLAGS}\" BOX64_DYNAREC_CALLRET=\"${BOX64_DYNAREC_CALLRET}\" BOX64_DYNAREC_NATIVEFLAGS=\"${BOX64_DYNAREC_NATIVEFLAGS}\""
fi
